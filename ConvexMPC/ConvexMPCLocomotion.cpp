...
void ConvexMPCLocomotion::run(ControlFSMData &data)
{
  ...
  world_position_desired[2] = 0.249; // 0.55; //.5;;;

  // std::cout << " get then foot location in world frame" << std::endl;
  for (int i = 0; i < 2; i++)
  {
    pFoot[i] = seResult.position + seResult.rBody.transpose() * (data._biped->getHip2Location(i) + data._legController->data[i].p);
    // std::cout << "pFoot                           "<<pFoot[i].transpose() << "\n";
    // std::cout << "seResult.position:              " << seResult.position.transpose() << "\n";
    // std::cout << "data._biped->getHip2Location(i):" << data._biped->getHip2Location(i).transpose() << "\n";
    // std::cout << "data._legController->data[i].p  " << data._legController->data[i].p.transpose() << "\n";
    // std::cout << "trans add                       " << (seResult.rBody.transpose() * (data._biped->getHip2Location(i) + data._legController->data[i].p)).transpose() << "\n";
  }
  ...
  for (int foot = 0; foot < 2; foot++)
  {
    ...
    if (swingState > 0) // foot is in swing
    {
      ...
      Vec3<double> hipOffset = {0.049, side * 0.075, -0.037};
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int *mpcTable, ControlFSMData &data, bool omniMode)
{
  ...
  if ((iterationCounter % 5) == 0)
  {
    ...
    q(2) += 0.25 * PI;//这里的正负号是否要参考xacro的joint origin？？？？？？
    q(3) -= 0.5 * PI;
    q(4) += 0.25 * PI;
